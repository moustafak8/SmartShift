...
export function ShiftCalendar({
  ...
  const formatTimeWithoutSeconds = (timeRange: string) => {
    return timeRange.replace(/(\d{2}:\d{2}):\d{2}/g, "$1");
  };
  ...
  const getUniqueShiftTimes = () => {
    ...
    return Array.from(timesMap.entries()).sort((a, b) =>
              ...
              >
                <div className="p-4 border-r border-[#E5E7EB]">
                  <div className="text-sm font-medium text-[#111827]">
                    {formatTimeWithoutSeconds(timeRange)}
                  </div>
                      ...
                      {shiftsForTime.length > 0 ? (
                        <div className="space-y-2">
                          {shiftsForTime.map((shift) => {
                            ...
                            const dayNum = String(day.getDate()).padStart(
                              2,
                              "0",
                            );
                            const dateStr = `${year}-${month}-${dayNum}`;
                            ...
                            const shiftType = shift.shift_type as
                              | "day"
                              | "evening"
                              | "night";
                            const shiftTypeAssignments =
                              shift.shift_type !== "rotating" && dayAssignments
