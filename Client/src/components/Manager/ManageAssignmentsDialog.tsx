...
import { useCreateAssignment, useDeleteAssignment, useAvailableEmployees } from "../../hooks/Manager/useAssignments";
...
export function ManageAssignmentsDialog({
    ...
    const [assignmentType, setAssignmentType] = useState<"regular" | "overtime" | "on_call">("regular");
    const [assignmentStatus, setAssignmentStatus] = useState<"assigned" | "pending" | "confirmed">("assigned");
    ...
    const { employees, isLoading: loadingEmployees } = useAvailableEmployees(departmentId || 0, date);
    ...
    const handleRemoveAssignment = (assignmentId: number, employeeName: string) => {
                    ...
                    <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-4">
                        ...
                        <div className="mt-3 flex items-center justify-between">
                            ...
                            <div className="w-32 bg-blue-200 rounded-full h-2">
                                <div
                                    className={`h-2 rounded-full transition-all ${assignedCount >= requiredCount
                                            ? "bg-green-500"
                            ...
                            <div className="space-y-3">
                                    ...
                                    >
                                        ...
                                        {availableEmployees.map((employee) => (
                                            <option key={employee.id} value={employee.id}>
                                                {employee.full_name} - {employee.preferred_shift_type}
                                            </option>
                                        ))}
                                    </select>
                                </div>
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => setAssignmentType(e.target.value as "regular" | "overtime" | "on_call")}
                                        disabled={isCreating}
                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#3B82F6] focus:border-transparent"
                                    >
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => setAssignmentStatus(e.target.value as "assigned" | "pending" | "confirmed")}
                                        disabled={isCreating}
                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#3B82F6] focus:border-transparent"
                                    >
